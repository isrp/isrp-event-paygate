...
class PayGateSettingsPage {
	...
	public function __construct(PayGate $paygate) {
		...
		$this->emailAdmin = new PayGateEmailAdminSetting('paygate_section_global');
	}
	...
	public function adminBCCMail() {
		return trim($this->emailAdmin->getValue()) ?: null;
	}
		...
		<tr onmouseover="document.getElementById('reg-<?php echo $row->id?>').style.display = 'block';"
			...
			<td>
				...
				<button title="Delete <?php echo $row->name?>" type="submit" onclick="return confirm('<?php
					printf(__('Remove test registration of %1$s?' /*translators: name*/, 'isrp-event-paygate'), $row->name)?>')"
